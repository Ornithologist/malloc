...
#define INITIALIZE_PTHREAD_MUTEX(mutex) \
    (memset(mutex, 0, sizeof(pthread_mutex_t)))
#define INITIALIZE_BIN_COUNTS(bin_ptr) \
    (memset(bin_ptr, 0, sizeof(uint16_t) * MAX_BINS))
...
typedef enum { USED = 0, UNUSED } arena_status_t;
...
#define ARENA_TO_HEAP_PTR(arena_ptr) \
    (((char*)arena_ptr) - sizeof(heap_header_t))
#define ARENA_TO_BLOCK_PTR(arena_ptr) \
    (((char*)arena_ptr) + sizeof(arena_header_t))
#define BLOCK_TO_ARENA_PTR(block_ptr) \
    (((char*)block_ptr) - sizeof(arena_header_t))
#define MMAP_BLOCK_TO_HEAP_PTR(block_ptr) \
    (((char*)block_ptr) - sizeof(heap_header_t))
...
void malloc_stats()
{
    ...
    do {
        ...
        printf("\t Total Size of Arena        : %ld KB\n",
               (get_total_arena_size(
                    (heap_header_t*)ARENA_TO_HEAP_PTR(arena_ptr)) /
                1024));
        printf("\t Total Bins                 : %d\n", MAX_BINS);
...
void pthread_atfork_prepare(void) { acquire_all_locks(); }

void pthread_atfork_parent(void) { release_all_locks(); }

void pthread_atfork_child(void) { release_all_locks(); }
...
void thread_destructor(void* ptr)
{
    ...
    if (ar_ptr->no_of_threads == 0) ar_ptr->arena_status = UNUSED;
...
int initialize_main_arena()
{
    ...
    if (malloc_initialized) return ret_val;
...
int initialize_thread_arena()
{
    ...
    uint16_t heap_size =
        sys_page_size + sizeof(arena_header_t) + sizeof(heap_header_t);
...
block_header_t* get_buddy_block_address(block_header_t* block)
{
    ...
    uint8_t block_index =
        ((block->block_base_address - (void*)block) / block_size);
    ...
    if (mem_ptr == NULL)
        ...
        while (heap_ptr) {
            if ((mem_ptr >= (void*)heap_ptr->heap_base_addr) &&
                (mem_ptr <= (void*)(((char*)heap_ptr->heap_base_addr) +
                                    heap_ptr->heap_size))) {
                ret_val = true;
                break;
            }
